        ...
        for d in domain_dict[unit]:
            ...
            if(checkValidAssignment(i,j,d)):
                ...
                for a in range(9):
                    for b in range(9):
                        if grid[a][b]==0:
                            localDomain_dict["Unit"+str(a)+str(b)]=updatedomain(grid,a,b,localDomain_dict)
...
for a in range(9):
    for b in range(9):
        if grid[a][b]==0:
            domain_dict["Unit"+str(a)+str(b)]=updatedomain(grid,a,b,domain_dict)
print("Initial grid")
for i in range(9):
    print(grid[i])
unit = MRV(domain_dict)
FOR_backtracking(grid,copy.deepcopy(domain_dict),int(unit[-2]),int(unit[-1]))
